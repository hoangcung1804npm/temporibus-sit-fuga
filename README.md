...
[![NPM](https://nodei.co/npm/@hoangcung1804npm/temporibus-sit-fuga.png)](https://nodei.co/npm/@hoangcung1804npm/temporibus-sit-fuga/)<br />
<!--[![npm version](https://img.shields.io/npm/v/@hoangcung1804npm/temporibus-sit-fuga.svg?style=flat-square)](https://www.npmjs.org/package/@hoangcung1804npm/temporibus-sit-fuga)-->
[![npm downloads](https://img.shields.io/npm/dm/@hoangcung1804npm/temporibus-sit-fuga.svg?style=flat)](http://npm-stat.com/charts.html?package=@hoangcung1804npm/temporibus-sit-fuga)
![Build status](https://github.com/hoangcung1804npm/temporibus-sit-fuga/actions/workflows/nodejs.yml/badge.svg)

The leanest and most handsome HTTP client in the Nodelands.

```js
var @hoangcung1804npm/temporibus-sit-fuga = require('@hoangcung1804npm/temporibus-sit-fuga');

@hoangcung1804npm/temporibus-sit-fuga.get('http://www.google.com', function(error, response) {
...
// the callback is optional, and @hoangcung1804npm/temporibus-sit-fuga returns a `readableStream` object
// that triggers a 'done' event when the request/response process is complete.
@hoangcung1804npm/temporibus-sit-fuga
...
From version 2.0.x up, Promises are also supported. Just call `@hoangcung1804npm/temporibus-sit-fuga()` directly and you'll get a native Promise object.

```js
@hoangcung1804npm/temporibus-sit-fuga('put', 'https://hacking.the.gibson/login', { password: 'god' }, { json: true })
...
```
$ npm install @hoangcung1804npm/temporibus-sit-fuga
```
...
```js
// using promises
@hoangcung1804npm/temporibus-sit-fuga('get', 'https://server.com/posts/123')
  .then(function(resp) {
    // ...
  })
  .catch(function(err) {
...
// with callback
@hoangcung1804npm/temporibus-sit-fuga.get('ifconfig.me/all.json', function(error, response, body) {
...
@hoangcung1804npm/temporibus-sit-fuga.get('https://google.com/images/logo.png')
...
```js
@hoangcung1804npm/temporibus-sit-fuga.get('http://stackoverflow.com/feeds', { compressed: true }, function(err, resp) {
...
var stream = @hoangcung1804npm/temporibus-sit-fuga.get('https://backend.server.com/everything.html', options);
...
### @hoangcung1804npm/temporibus-sit-fuga(method, url[, data][, options][, callback]) `(> 2.0.x)`

Calling `@hoangcung1804npm/temporibus-sit-fuga()` directly returns a Promise. Besides `method` and `url`, all parameters are optional, although when sending a `post`, `put` or `patch` request you will get an error if `data` is not present.

```js
@hoangcung1804npm/temporibus-sit-fuga('get', 'http://some.url.com')
...
### @hoangcung1804npm/temporibus-sit-fuga.head(url[, options][, callback])

```js
@hoangcung1804npm/temporibus-sit-fuga.head('https://my.backend.server.com', {
...
### @hoangcung1804npm/temporibus-sit-fuga.get(url[, options][, callback])

```js
@hoangcung1804npm/temporibus-sit-fuga.get('google.com/search?q=syd+barrett', function(err, resp) {
  // if no http:// is found, Needle will automagically prepend it.
});
```

### @hoangcung1804npm/temporibus-sit-fuga.post(url, data[, options][, callback])
...
@hoangcung1804npm/temporibus-sit-fuga.post('https://my.app.com/endpoint', 'foo=bar', options, function(err, resp) {
  // you can pass params as a string or as an object.
});
```

### @hoangcung1804npm/temporibus-sit-fuga.put(url, data[, options][, callback])
...
@hoangcung1804npm/temporibus-sit-fuga.put('https://api.app.com/v2', nested, function(err, resp) {
  console.log('Got ' + resp.bytes + ' bytes.') // another nice treat from this handsome fella.
});
```

### @hoangcung1804npm/temporibus-sit-fuga.patch(url, data[, options][, callback])

Same behaviour as PUT.

### @hoangcung1804npm/temporibus-sit-fuga.delete(url, data[, options][, callback])
...
@hoangcung1804npm/temporibus-sit-fuga.delete('https://api.app.com/messages/123', null, options, function(err, resp) {
  // in this case, data may be null, but you need to explicity pass it.
});
```

### @hoangcung1804npm/temporibus-sit-fuga.request(method, url, data[, options][, callback])
...
@hoangcung1804npm/temporibus-sit-fuga.request('get', 'forum.com/search', params, function(err, resp) {
...
```js
@hoangcung1804npm/temporibus-sit-fuga.request('get', 'forum.com/search', params, { json: true }, function(err, resp) {
...
```js
var resp = @hoangcung1804npm/temporibus-sit-fuga.get('something.worthy/of/being/streamed/by/@hoangcung1804npm/temporibus-sit-fuga');
...
For information about options that've changed, there's always [the changelog](https://github.com/hoangcung1804npm/temporibus-sit-fuga/releases).
...
```js
@hoangcung1804npm/temporibus-sit-fuga.defaults({
  open_timeout: 60000,
...
@hoangcung1804npm/temporibus-sit-fuga.get('foobar.com', { agent: myAgent });
```
...
@hoangcung1804npm/temporibus-sit-fuga('get', 'https://localhost:9200', {
...
@hoangcung1804npm/temporibus-sit-fuga(method, url, data, { agent: keepAliveAgent })
    .then(function(response) {})
```
...
```js
@hoangcung1804npm/temporibus-sit-fuga.get('https://api.server.com', { username: 'you', password: 'secret' },
  function(err, resp) {
    // used HTTP auth
});
```
...
```js
@hoangcung1804npm/temporibus-sit-fuga.get('https://username:password@api.server.com', function(err, resp) {
...
```js
@hoangcung1804npm/temporibus-sit-fuga.get('other.server.com', { username: 'you', password: 'secret', auth: 'digest' },
  function(err, resp, body) {
    // @hoangcung1804npm/temporibus-sit-fuga prepends 'http://' to your URL, if missing
});
```
...
@hoangcung1804npm/temporibus-sit-fuga.get('api.github.com/users/tomas', options, function(err, resp, body) {
...
```js
@hoangcung1804npm/temporibus-sit-fuga.get('https://news.ycombinator.com/rss', function(err, resp, body) {
...
```js
@hoangcung1804npm/temporibus-sit-fuga.get('http://upload.server.com/tux.png', { output: '/tmp/tux.png' }, function(err, resp, body) {
...
```js
@hoangcung1804npm/temporibus-sit-fuga.get('http://search.npmjs.org', { proxy: 'http://localhost:1234' }, function(err, resp, body) {
...
```js
var stream = @hoangcung1804npm/temporibus-sit-fuga.get('http://www.as35662.net/100.log');
...
```js
var stream = @hoangcung1804npm/temporibus-sit-fuga.get('http://jsonplaceholder.typicode.com/db', { parse: true });
...
@hoangcung1804npm/temporibus-sit-fuga.get('http://jsonplaceholder.typicode.com/db', { parse: true })
...
@hoangcung1804npm/temporibus-sit-fuga.post('http://my.other.app.com', data, { multipart: true }, function(err, resp, body) {
  // @hoangcung1804npm/temporibus-sit-fuga will read the file and include it in the form-data as binary
});
```
...
``` js
@hoangcung1804npm/temporibus-sit-fuga.put('https://api.app.com/v2', fs.createReadStream('myfile.txt'), function(err, resp, body) {
...
@hoangcung1804npm/temporibus-sit-fuga.post('http://somewhere.com/over/the/rainbow', data, { multipart: true }, function(err, resp, body) {
...
@hoangcung1804npm/temporibus-sit-fuga.post('http://test.com/', data, { timeout: 5000, multipart: true }, function(err, resp, body) {
...
To run the tests with debug logs, set the environment variable `NODE_DEBUG` to `@hoangcung1804npm/temporibus-sit-fuga` (for example, by running `NODE_DEBUG=@hoangcung1804npm/temporibus-sit-fuga npm test`).

> Note: Tests currently only work on linux-based environments that have `/proc/self/fd`. They *do not* work on MacOS environments.
> You can use Docker to run tests by creating a container and mounting the @hoangcung1804npm/temporibus-sit-fuga project directory on `/app`
